...
//验证码的宽度
	...
	{
		$this->doimage();
		$_SESSION['checkcode'][(int)$id] = $this->code;
	}

	
	function check($id, $code=null)
	{
		if( isset($_SESSION['checkcode']) && is_array($_SESSION['checkcode']) ){
			$id = (int)$id;
			if( array_key_exists($id, $_SESSION['checkcode']) ){
				if( $_SESSION['checkcode'][$id] === true ){
					return true;
				}
				if( strcasecmp($_SESSION['checkcode'][$id], $code) == 0 ){
					$_SESSION['checkcode'][$id] = true;
					return true;
				}
			}
		}
