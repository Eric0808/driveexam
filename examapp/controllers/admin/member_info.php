 ...
 *@database 
 */
final class Member_info extends CI_Controller
{
	...
	public function index()
	{
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			$sUname = !empty($_POST['ser_uname']) ? " `username` LIKE '%" . str_replace("\'", "\"", trim($_POST['ser_uname'])) ."%' "  : "";
			if(empty($sUname))
			{$sEmail = !empty($_POST['ser_email']) ? " `email`='".$_POST['ser_email']."'" : "";}
			else{$sEmail = !empty($_POST['ser_email']) ? " AND `email`=" . intval($_POST['ser_email']) : "";}
			if(!(empty($sUname) && empty($sEmail) )){
			$where = ' WHERE ' . $sUname . $sEmail;}
		
		}
		$memberList = $this->member_model->Getallmember($cPage, 20, 10 , $where, ' ORDER BY m.userid DESC ');
	...
	public function deleteby_id() {
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			if(!empty($_POST['ids'])){
				foreach($_POST['ids'] as $id){
					$id = intval($id);
					$result = $this->member_model->deluser_byID($id);
					if(count($result)!=2 || !$result[0] || !$result[1]){
