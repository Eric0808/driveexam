...
//        document.write(document.cookie.substring(c_start,c_end)+"<br>");
	...
	{	
		...
		if( typeof this.data[this.currentid] === 'undefined' ){
			...
			document.cookie="activetime=" +this.currentid+";path=/;expires="+exdate.toGMTString();
			//var actid=getCookie('activetime')
			//
	...
	this.addError = function(id, showmsg){
		...
		ajax.onreadystatechange = function(){
			...
			if( ajax.status===200 && ajax.responseText!=='' ){
				if(ajax.responseText=='stop'){
					EV_modeAlert('ErrTipAreaHtml');
				}
				else
