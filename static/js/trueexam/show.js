...
function _show()
{
	...
	this.create = function(data, id){
		...
		this.set('question', '<b id="cnum">'+topic.getNumber(this.id)+'.</b>&nbsp;'+this.data.question);
	...
	this.setButton = function(name, data)
	{
		...
		a.onclick = function(){input.click();
					...
					if( obj != null ){//alert(topic.getNumber(this.id));
						//obj.appendChild(input.getAttribute('data-name'));
						//this.setText(obj, input.getAttribute('data-name')); 
						//var intext = obj.innerText;
						var num_id = document.getElementById('cnum').innerText;
						//alert(num_id);
						num_id ='bn_'+ num_id.substring(0,num_id.lastIndexOf('.'));
						//alert(num_id);
						var obj_dd =document.getElementById(num_id);
						//alert(obj_dd);
						obj_dd.innerHTML=input.getAttribute('data-name');
					}
				}
		};
