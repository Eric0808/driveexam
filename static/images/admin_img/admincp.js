 ...
 * license terms
 * 
 * $Id: admincp.js 5613 2010-12-27 01:22:47Z menglingmin $
 */
	...
	if (op == 0) {
		if (obj.scrollHeight > 70) {
			if (obj.style.position == 'absolute') {
				...
				obj.parentNode.style.height = (obj.scrollHeight < 300 ? 20 + parseInt(obj.scrollHeight): 300) + 'px';
			}
		}
	} else {
		if (obj.style.position == 'absolute') {
			...
			$(obj.parentNode).css({"vertical-align":"top"});
...
function delete_item_in_inputtag(display_obj, itemid, input_id) {
	$(display_obj).parent().fadeOut();
